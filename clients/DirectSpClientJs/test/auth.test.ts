namespace directSp {
  ...
  test('autoSignin on', async () => {
    //init
    const client = directSp.TestUtil.CreateDspClient(null, {
      auth: {
    ...
    if (!(<DirectSpControlFake>client.control).called_navigateUri)
      throw new DirectSpError("SignIn have not called!");
  ...
  test('login by redirect and refresh token', async () => {
    ...
    const client = TestUtil.CreateDspClient((request) => {
      ...
      if (request.data && request.data.grant_type == "authorization_code") {
        ...
        if (!request.headers) throw new DirectSpError("headers does not contain data!");
        ...
        throw new DirectSpError("Unexpected request!");
      }
